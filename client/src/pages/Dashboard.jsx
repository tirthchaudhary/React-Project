import { FilePenLineIcon, PencilIcon, PlusIcon, TrashIcon, UploadCloudIcon, XIcon, UploadCloud, NavigationOff, LoaderCircleIcon } from "lucide-react";
...
const Dashboard = () => {
  ...
  const uploadResume = async (event) => {
    ...
    try {
      ...
      console.log("Extracted PDF Text:", resumeText); // Check console log to inspect extracted text!
      // 2. Guard check: Make sure text was actually extracted
      if (!resumeText || !resumeText.trim()) {
        toast.error("Could not extract text from this PDF. Please upload a PDF with selectable text (not scanned images).");
        setIsLoading(false);
        return;
      }
      // 3. Send to backend
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-200">
      <div className="max-w-7xl mx-auto px-4 py-8">
        <p className="text-2xl font-medium mb-6 bg-gradient-to-r from-slate-600 to-slate-700 dark:from-slate-200 dark:to-slate-400 bg-clip-text text-transparent sm:hidden">Welcome, Joe Doe</p>
        ...
        <div className="grid grid-cols-2 sm:flex flex-wrap gap-4">
          {allResumes.map((resume, index) => {
            ...
            return (
              <button onClick={() => navigate(`/app/builder/${resume._id}`)} key={index} className="relative w-full sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 border group hover:shadow-lg transition-all duration-300 cursor-pointer"
                style={{ background: `linear-gradient(135deg,${baseColor}10,${baseColor}10,${baseColor}40)`, borderColor: baseColor + '40' }}
              >
            ...
            <div onClick={e => e.stopPropagation()} className="relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6">
              ...
              <div>
                <label htmlFor="resume-input" className="block text-sm text-slate-700">
                  Select resume file
                  <div className="flex flex-col items-center gap-2 border group text-slate-400 bordr-slate-400 bordere-dashed rounded-md p-4 py-10 my-4 hover:border-r-green-500 hover:text-green-700 cursor-pointer transition-colors">
                    {resume ? (
              ...
              <button disabled={isLoading} type="submit" className="w-full py-2 bg-green-600 text-white rounded hover:bg-green-700 transition-colors flex items-center justify-center gap-2">
                {isLoading && <LoaderCircleIcon className="animate-spin size-4 text-white flex items-center" />}
                {isLoading ? 'uploading...' : 'Upload Resume'}
              </button>
              <XIcon className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 cursor-pointer transition-colors" onClick={() => {
                setshowUploadResume(false); setTitle(''); setResume(false)
              }} />
            </div>
          </form>
        )
